...
import { IFiles, IPatient, IUserExtraData, IUserExtraImageType } from 'src/interfaces';
...
export class PatientService extends BaseApiService<IPatient> {
  ...
  getUserDataImage(pinfl: string, birth_date: string): Observable<IFiles> {
    return this.http.get<IFiles>(`${environment.apiUrl}/staff/user/data/image`, {params: {pinfl, birth_date}});
  }
}
